...
def plot_confusion_matrix(y_true, y_pred, classes, title, accuracy=None, save_path=None):
...
def create_label_comparison_table(
    ...
    rows.append(
        "| Sampling of points | "
        + " | ".join(map(str, sampling_labels))
        + " |"
    )
    rows.append(
        "| Classical NC | " + " | ".join(map(str, classical_labels)) + " |"
    )
    rows.append(
        "| QNC (no mitigation) | "
        + " | ".join(map(str, quantum_no_mit_labels))
        + " |"
    )
    rows.append(
        "| QNC (with mitigation) | "
        + " | ".join(map(str, quantum_mit_labels))
        + " |"
    )
    ...
    classical_acc = (
        np.mean(np.array(classical_labels) == np.array(sampling_labels)) * 100
    )
    quantum_no_mit_acc = (
        np.mean(np.array(quantum_no_mit_labels) == np.array(classical_labels))
        * 100
    )
    quantum_mit_acc = (
        np.mean(np.array(quantum_mit_labels) == np.array(classical_labels))
        * 100
    )
...
def plot_iris_pca_scatter(
    ...
    if centroids_pca is not None:
        for i, centroid in enumerate(centroids_pca):
            ax.scatter(
                ...
                edgecolors='black',
                linewidths=2,
                label=f'Centroid {class_names[i]}' if i < len(class_names) else f'Centroid {i}',
            )

    # Create legend
    from matplotlib.lines import Line2D
    legend_elements = []
    for i, (color, name) in enumerate(zip(colors[:len(unique_classes)], class_names[:len(unique_classes)])):
        legend_elements.append(
            Line2D([0], [0], marker='o', color='w', markerfacecolor=color,
                   markeredgecolor=color, markersize=10, label=name)
        )
    ...
    mit_str = "(before error mitigation)" if before_mitigation else "(after error mitigation)"
...
def plot_c_exp_vs_c_sim(
    ...
    # Scatter plots
    ax.scatter(c_sim_values, c_exp_no_mitigation, 
               c='#3498db', marker='o', s=50, alpha=0.6, label='No mitigation')
    ax.scatter(c_sim_values, c_exp_with_mitigation,
               c='#e74c3c', marker='s', s=50, alpha=0.6, label='After mitigation')

    # Linear fits
    # No mitigation fit
    mask_no_mit = ~np.isnan(c_exp_no_mitigation) & ~np.isnan(c_sim_values)
    if mask_no_mit.sum() > 1:
        coeffs_no_mit = np.polyfit(c_sim_values[mask_no_mit], c_exp_no_mitigation[mask_no_mit], 1)
        x_fit = np.linspace(0, 1, 100)
        y_fit_no_mit = coeffs_no_mit[0] * x_fit + coeffs_no_mit[1]
        ax.plot(x_fit, y_fit_no_mit, '#3498db', linestyle='-', linewidth=2,
    ...
    if mask_mit.sum() > 1:
        coeffs_mit = np.polyfit(c_sim_values[mask_mit], c_exp_with_mitigation[mask_mit], 1)
        y_fit_mit = coeffs_mit[0] * x_fit + coeffs_mit[1]
        ax.plot(x_fit, y_fit_mit, '#e74c3c', linestyle='-', linewidth=2,
...
def plot_synthetic_results(
    ...
    ax1.errorbar(x, dist_ratios, yerr=dist_ratio_stds, fmt='o', 
                 color='#3498db', capsize=5, markersize=8)
    ax1.axhline(y=1.0, color='gray', linestyle='--', alpha=0.5)
    ax1.set_ylabel(r'$l_{exp} / l_{sim}$')
    ax1.set_xticks(x)
    ax1.set_xticklabels([])
    ...
    ax2.bar(x - width/2, before_err, width, label='Before mitigation', color='#3498db', alpha=0.8)
    ax2.bar(x + width/2, after_err, width, label='After mitigation', color='#e74c3c', alpha=0.8)
