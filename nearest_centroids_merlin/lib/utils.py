...
def get_angles(x):
    ...
    # remove nan and enforce 1D array
    thetas = np.nan_to_num(thetas, nan=0.0)
    # thetas is (1, k) → reshape to (k,)
...
def create_circuit(n):
    ...
    angle_y, wires_y = np.flip(get_angles(y)[0], 0), np.flip(
        get_angles(y)[1], 0
    )
    circuit = pcvl.Circuit(n)
    k = 0
    for theta, (i, j) in zip(angle_x, wires_x):
    ...
    for theta, (i, j) in zip(angle_y, wires_y):
        i = int(i)
        j = int(j)
